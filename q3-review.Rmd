...
a. This maps all of $\mathbb{R}^2$ to a line. Therefore it is not one-to-one, nor onto, and so it is not invertible. This means that 0 is an eigenvector. Any vector that is already on the line must stay on the line, so it is an eigenvector, but we don't know its eigenvalue. Thus, the eigenvalues are $\lambda_1 = 1$ and  $\lambda_2=0$.
